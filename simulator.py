...
class GameSimulator:
    ...
    def run(self):
        while self.running:
            ...
            state1 = self._get_state(self.tank1, self.tank2)
            state2 = self._get_state(self.tank2, self.tank1)

            action1 = self.agent1.select_action(state1)
            action2 = self.agent2.select_action(state2)

            shot1, wall_hit1 = self.tank1.update(action1.item(), self.walls)
            shot2, wall_hit2 = self.tank2.update(action2.item(), self.walls)
            if shot1: self._play_sound('shoot')
            if shot2: self._play_sound('shoot')

            self.bullets.update(self.walls, self.screen.get_rect())

            # Check for powerup spawn
            current_time = pygame.time.get_ticks()
            if not self.powerups and current_time > self.next_powerup_spawn_time:
                self._spawn_powerup()

            # Check for tank-powerup collisions
            ...
            reward1 = -0.01
            reward2 = -0.01
            if wall_hit1: reward1 -= 1
            if wall_hit2: reward2 -= 1

            done = False
            hit_info = self._check_collisions()
            if hit_info:
                done = True
                winner, loser = hit_info
                if winner == self.tank1:
                    reward1 += 500; reward2 -= 500
                    self.score[1] += 1
                else:
                    reward1 -= 500; reward2 += 500
                    self.score[2] += 1
                ...
                print(f"Round {self.episode_count} Over. Winner: Tank {winner.agent_id}")
                self.reset_round()
            ...
            next_state1 = self._get_state(self.tank1, self.tank2) if not done else None
            next_state2 = self._get_state(self.tank2, self.tank1) if not done else None

            # N-step learning logic
            self._process_n_step_buffer(self.agent1, self.agent1_n_step_buffer, state1, action1, reward1, done)
            self._process_n_step_buffer(self.agent2, self.agent2_n_step_buffer, state2, action2, reward2, done)
            ...
            for tank in hit_tanks:
                if bullet.owner != tank:
                    ...
                    if tank.shield_active:
                        ...
                        # Don't end the round, but the bullet is gone
                    else:
                        return bullet.owner, tank # Return winner, loser
        return None

    def _check_bullet_wall_collisions(self):
        # Bullets are destroyed upon hitting a wall, and damage the wall
    ...
    def _process_n_step_buffer(self, agent, buffer, state, action, reward, done):
        # The experience stored is from the perspective of state_t
        ...
        n_step_reward = 0
        for i, exp in enumerate(reversed(buffer)):
            n_step_reward += (GAMMA**i) * exp[2]

        # Get the initial state and action
        initial_state, initial_action, _ = buffer[0]

        # Get the final state for the transition
        ...
        if done:
            # On episode end, clear buffer by processing remaining transitions
            while len(buffer) > 1:
                buffer.popleft()
                # Recalculate for the shortened buffer
                n_step_reward = 0
                for i, exp in enumerate(reversed(buffer)):
                    n_step_reward += (GAMMA**i) * exp[2]
                initial_state, initial_action, _ = buffer[0]
    ...
    def reset_round(self):
        self.select_new_map() # Select a new map for the next round

        # Clear n-step buffers
        self.agent1_n_step_buffer.clear()
        self.agent2_n_step_buffer.clear()

        spawn_points = self._get_spawn_points()
        self.tank1.pos = pygame.math.Vector2(spawn_points[0])
        self.tank1.angle = 0
        self.tank2.pos = pygame.math.Vector2(spawn_points[1])
